...
export abstract class ToolOutput {
  ...
  export interface ToolEvents<TInput = any, TOutput = any> {
    onStart?: (input: TInput, context: ExecutionContext, options: RunOptions) => Promise<void>;
    onSuccess?: (output: TOutput, input: TInput, context: ExecutionContext, options: RunOptions) => Promise<void>;
    onError?: (error: ToolError, input: TInput, context: ExecutionContext, options: RunOptions) => Promise<void>;
    onRetry?: (error: ToolError, attempt: number, input: TInput, context: ExecutionContext, options: RunOptions) => Promise<void>;
  }
  ...
  > {
    ...
    constructor(
      ...
      options?: TOptions,
      protected readonly events?: ToolEvents<TInput, TOutput>
    ) {
    ...
    private validateInput(input: unknown): asserts input is TInput {
    ...
    async run(rawInput: unknown, options: RunOptions = {}): Promise<TOutput> {
        try {
          ...
          await this.events?.onError?.(toolError, rawInput as TInput, this.context, options);
          throw toolError;
        }
    }
  }
  ...
  export class DynamicTool<TInput, TOutput extends ToolOutput> extends Tool<TInput, TOutput> {
    constructor(
      name: string,
      description: string,
