...
const loggerConfig: LoggingConfig = {
  destination: path.join(process.cwd(), `${ChatBot.getName()}.log`)
};
...
ChatBot.run(loggerConfig);
ChatBot.registerStreamCallback((delta: string) => {
...
function setupResponseHandler(session: any) {
    session.onEvent((response: any) => {
        ...
        if (typeof response === 'string') {
            ...
            if ('clarification' in response) {
                ...
                if (questions) {
                    console.log(`${ChatBot.getName()}: ${questions.join('\n')}`);
                }
            } else if ('confirmation' in response) {
                const { prompt, options } = response.confirmation;
                if (prompt) {
                    console.log(`${ChatBot.getName()}: ${prompt}`);
                    if (options) {
                ...
                if (reason) {
                    console.log(`${ChatBot.getName()} Error: ${reason}`);
                    if (suggestedAction) {
    ...
    session.onException((response: any) => {
        console.log(`${ChatBot.getName()} Error:`, JSON.stringify(response, null, 2));
    });
...
async function handleChat(session: any): Promise<void> {
    while (true) {
        ...
        if (userInput.toLowerCase().trim() === '/exit') {
            console.log("Thank you for using the ChatBot. Shutting down...");
...
async function chatLoop(): Promise<void> {
    try {
        ...
        const session = await ChatBot.createSession("user", initialInput);
