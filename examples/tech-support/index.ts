import { TechSupportAgent } from './TechSupportAgent';
...
const loggerConfig: LoggingConfig = {
  destination: path.join(process.cwd(), `${TechSupportAgent.getName()}.log`)
};
...
TechSupportAgent.run(loggerConfig);
TechSupportAgent.registerStreamCallback((delta: string) => {
...
async function handleInitialInput(): Promise<string> {
    ...
    do {
        ...
        if (input.toLowerCase().trim() === '/exit') {
            console.log("Thank you for using the TechSupportAgent. Goodbye!");
            process.exit(0);
        }
...
function setupResponseHandler(session: any) {
    session.onEvent((response: any) => {
        ...
        if (typeof response === 'string') {
            ...
            if ('clarification' in response) {
                ...
                if (questions) {
                    console.log(`${TechSupportAgent.getName()}: ${questions.join('\n')}`);
                }
            } else if ('confirmation' in response) {
                const { prompt, options } = response.confirmation;
                if (prompt) {
                    console.log(`${TechSupportAgent.getName()}: ${prompt}`);
                    if (options) {
                ...
                if (reason) {
                    console.log(`${TechSupportAgent.getName()} Error: ${reason}`);
                    if (suggestedAction) {
    ...
    session.onException((response: any) => {
        console.log(`${TechSupportAgent.getName()} Error:`, JSON.stringify(response, null, 2));
    });
...
async function handleChat(session: any): Promise<void> {
    while (true) {
        ...
        if (userInput.toLowerCase().trim() === '/exit') {
            console.log("Thank you for using the TechSupportAgent. Shutting down...");
            await TechSupportAgent.shutdown();
            break;
        }
...
async function chatLoop(): Promise<void> {
    try {
        console.log("~~~ Welcome to the TechSupportAgent ~~~");
        console.log("Type '/exit' to end the conversation.");

        const initialInput = await handleInitialInput();
        const session = await TechSupportAgent.createSession("user", initialInput);
