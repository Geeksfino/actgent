...
## Testing Agents with Simulated Users

Actgent provides a flexible test harness for simulating user-agent conversations using any OpenAI-compatible LLM as the simulated user. This enables robust, automated evaluation of agent endpoints.

### Example: Running the Test Harness

You can use the provided [test/test-agent.ts](test/test-agent.ts) script to test any agent that exposes an OpenAI-style or Gemini-style WebSocket API. The simulated user messages are generated by a local LLM (OpenAI, Gemini, Groq, DeepSeek, etc.), and sent to the target agent.

**Example command:**

```bash
NO_PROXY=localhost bun run test/test-agent.ts \
  --llm-api-key <YOUR_LLM_API_KEY> \
  --llm-model <YOUR_LLM_MODEL> \
  --llm-url <YOUR_LLM_API_URL> \
  --agent-ws-url <AGENT_WS_URL> \
  --user-max-tokens 500 \
  --user-temperature 0.5 \
  --num-rounds 4 \
  --scenario "You are a connoisseur. You like chatting about Italian food"
```

- Any OpenAI-compatible LLM can be used for the simulated user (e.g., OpenAI, Gemini, Groq, DeepSeek, etc.).
- The scenario string customizes the simulated user's persona and interests.
- The conversation is fully automated, with clear round-by-round output.

See [test/test-agent.ts](test/test-agent.ts) for advanced configuration options and further documentation.
